...
import { getStudentsWithScores, saveActivityScores } from "@/actions/activities";
...
const ScoresModal: React.FC<ScoresModalProps> = ({
  ...
  const schema = useMemo(
    () => buildSchema(activity?.totalScore ?? 0),
    [activity?.totalScore],
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      const rows = await getStudentsWithScores(activity.id, subjectId);
      reset({
        scores: rows.map((s) => ({
          studentId: s.id,
          fullName: s.fullName,
    ...
    load();
  }, [isOpen, activity, subjectId, reset]);
  ...
  // Map each field to its original index for register(), then filter by search
  ...
  const getRate = (originalIndex: number): string => {
    ...
    if (isNaN(num)) return "—";
    return `${Math.round((num / activity.totalScore) * 100)}%`;
  };
  ...
  const getRateClass = (originalIndex: number): string => {
    ...
    const ratio = Number(raw) / activity.totalScore;
  ...
  const onSubmit = async (data: ScoresFormValues) => {
    ...
    const result = await saveActivityScores(activity.id, subjectId, entries);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-7 py-5 border-b border-slate-100 shrink-0">
          <div>
            <h2 className="text-base font-bold text-slate-800">
              Scores — {activity.title}
            </h2>
            <p className="text-xs text-slate-400 mt-0.5">
              Max score: {activity.totalScore} · {activity.type} · {activity.activityDate}
            </p>
          </div>
          <button
            type="button"
        ...
        {/* Table */}
        <form id="scores-form" onSubmit={handleSubmit(onSubmit)} noValidate>
          {/* Hidden fields to keep all studentId/fullName/gender registered */}
          {fields.map((field, index) => (
            <input key={field.id} type="hidden" {...register(`scores.${index}.studentId`)} />
          ))}
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-white">
                <tr className="border-b border-gray-100">
                  {["#", "Student Name", "Gender", `Score (/ ${activity.totalScore})`, "Rate"].map(
                    (col) => (
                      <th
                        key={col}
              ...
              <tbody className="divide-y divide-gray-50">
                {loading ? (
                  ...
                  paginatedFields.map(({ originalIndex, fullName, gender }, displayIndex) => {
                    ...
                    return (
                      <tr key={originalIndex} className="hover:bg-gray-50">
                        ...
                        <td className={`px-4 py-3 text-sm ${getRateClass(originalIndex)}`}>
