...
interface AddActivityModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: ActivityFormValues) => Promise<boolean>;
}
...
const AddActivityModal = ({ isOpen, onClose, onSave }: AddActivityModalProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      reset({ title: "", type: undefined, total: undefined, date: "" });
    }
  }, [isOpen, reset]);
  ...
  const submitHandler = async (data: ActivityFormValues) => {
    ...
    if (success) reset({ title: "", type: undefined, total: undefined, date: "" });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
        ...
        <div className="flex items-center justify-between px-7 py-5 border-b border-slate-100">
          <div>
            <h2 className="text-base font-bold text-slate-800">New Activity</h2>
