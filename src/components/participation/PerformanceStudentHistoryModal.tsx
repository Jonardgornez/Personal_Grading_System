...
interface PerformanceStudentHistoryModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const PerformanceStudentHistoryModal: React.FC<
  PerformanceStudentHistoryModalProps
> = ({ isOpen, onClose, student, autoPrint = false }) => {
  ...
  const statCards = [
    {
      label: "Entries Scored",
      value: `${student.scoredCount} / ${student.totalPerformances}`,
    },
  ...
  return (
    <>
      ...
      <div ref={printRef} className="bg-white p-8 w-200">
        ...
        <p className="text-sm text-slate-400 mb-5">{student.gender} · Performance History</p>
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            { label: "Entries Scored", value: `${student.scoredCount} / ${student.totalPerformances}` },
            { label: "Total Earned", value: `${student.totalEarned} pts` },
            { label: "Total Possible", value: `${student.totalPossible} pts` },
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="border-b-2 border-slate-200">
              {["#", "Performance", "Date", "Max", "Score", "Rate"].map((col) => (
          ...
          <tbody>
            {student.scores.map((s, i) => {
              ...
              return (
                <tr key={s.performanceId} className="border-b border-slate-100">
                  <td className="px-3 py-2 text-slate-400">{i + 1}</td>
                  <td className="px-3 py-2 text-slate-800 font-medium">{s.performanceTitle}</td>
                  <td className="px-3 py-2 text-slate-500">{s.performanceDate}</td>
        ...
        <div className="flex items-center justify-between px-7 py-5 border-b border-slate-100 shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {student.gender} · Performance History
            </p>
          </div>
          <button
            type="button"
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-white">
              <tr className="border-b border-gray-100">
                {["#", "Performance", "Date", "Max", "Score", "Rate"].map(
                  (col) => (
                    <th
                      key={col}
            ...
            <tbody className="divide-y divide-gray-50">
              {student.scores.length === 0 ? (
                <tr>
                  ...
                  >
                    No performance entries in this subject.
                  </td>
                </tr>
              ) : (
                student.scores.map((s, index) => {
                  ...
                  const rate = hasScore
                    ...
                    : "—";
                  return (
                    <tr key={s.performanceId} className="hover:bg-gray-50">
...
export default PerformanceStudentHistoryModal;
