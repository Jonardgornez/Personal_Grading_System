...
interface AddParticipationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: ParticipationFormValues) => Promise<boolean>;
}
...
const AddParticipationModal = ({
  isOpen,
  onClose,
  onSave,
}: AddParticipationModalProps) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const submitHandler = async (data: ParticipationFormValues) => {
        ...
        <div className="flex items-center justify-between px-7 py-5 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Fill in the details to add a new participation entry.
            </p>
          </div>
          <button
            type="button"
