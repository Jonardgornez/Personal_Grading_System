...
import { getPerformanceDetails } from "@/actions/performances";
import type { PerformanceRow, PerformanceDetails } from "@/types/domain";

interface ViewParticipationModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const ViewParticipationModal: React.FC<ViewParticipationModalProps> = ({
  isOpen,
  onClose,
  performance,
  subjectId,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      const data = await getPerformanceDetails(performance.id, subjectId);
      setDetails(data);
      setLoading(false);
    };

    load();
  }, [isOpen, performance, subjectId]);
    ...
    },
    {
      ...
      value:
        details?.avgScore != null
          ? `${details.avgScore} / ${performance.totalScore}`
          : "—",
      sub:
        details?.avgScore != null
          ? `${Math.round((details.avgScore / performance.totalScore) * 100)}%`
          : "no scores yet",
    },
    {
      label: "Highest",
        ...
        <div className="flex items-center justify-between px-7 py-5 border-b border-slate-100 shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {performance.performanceDate} · Max score: {performance.totalScore}
            </p>
          </div>
          <button
            type="button"
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                paginated.map((s, index) => {
                  ...
                  const rate = hasScore
                    ? `${Math.round((s.score! / performance.totalScore) * 100)}%`
                    : "—";
                  return (
                    <tr key={s.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-700">
                        {hasScore ? `${s.score} / ${performance.totalScore}` : "—"}
                      </td>
                      <td
                        className={`px-4 py-3 text-sm ${
                          hasScore
                            ? getRateClass(s.score!, performance.totalScore)
                            : "text-gray-400"
                        }`}
                      >
                        {rate}
...
export default ViewParticipationModal;
