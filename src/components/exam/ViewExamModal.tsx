...
import { getExamDetails } from "@/actions/exams";
import type { ExamRow, ExamDetails } from "@/types/domain";
...
const ViewExamModal: React.FC<ViewExamModalProps> = ({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      const data = await getExamDetails(exam.id, subjectId);
      setDetails(data);
      setLoading(false);
    };
    ...
    },
    {
      ...
      value:
        details?.avgScore != null
          ? `${details.avgScore} / ${exam.totalScore}`
          : "—",
      sub:
        details?.avgScore != null
          ? `${Math.round((details.avgScore / exam.totalScore) * 100)}%`
          : "no scores yet",
    },
    {
      label: "Highest",
        ...
        <div className="flex items-center justify-between px-7 py-5 border-b border-slate-100 shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {exam.examDate} · Max score: {exam.totalScore}
            </p>
          </div>
          <button
            type="button"
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                paginated.map((s, index) => {
                  ...
                  const rate = hasScore
                    ? `${Math.round((s.score! / exam.totalScore) * 100)}%`
                    : "—";
                  return (
                    <tr key={s.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-700">
                        {hasScore ? `${s.score} / ${exam.totalScore}` : "—"}
                      </td>
                      <td
                        className={`px-4 py-3 text-sm ${
                          hasScore ? getRateClass(s.score!, exam.totalScore) : "text-gray-400"
                        }`}
                      >
                        {rate}
                      </td>
