...
import { getStudentsWithExamScores, saveExamScores } from "@/actions/exams";
...
const ExamScoresModal: React.FC<ExamScoresModalProps> = ({
  isOpen,
  onClose,
  exam,
  subjectId,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      const rows = await getStudentsWithExamScores(exam.id, subjectId);
      reset({
        scores: rows.map((s) => ({
          studentId: s.id,
          fullName: s.fullName,
  ...
  const getRate = (originalIndex: number): string => {
    ...
    if (isNaN(num)) return "—";
    return `${Math.round((num / exam.totalScore) * 100)}%`;
  };

  const getRateClass = (originalIndex: number): string => {
    if (!exam) return "text-gray-400";
    const raw = watchedScores?.[originalIndex]?.score;
    if (!raw || raw === "") return "text-gray-400";
    const ratio = Number(raw) / exam.totalScore;
  ...
  const onSubmit = async (data: ScoresFormValues) => {
    ...
    const result = await saveExamScores(exam.id, subjectId, entries);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-7 py-5 border-b border-slate-100 shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Max score: {exam.totalScore} · {exam.examDate}
            </p>
          </div>
          <button
            type="button"
        ...
        {/* Table */}
        <form id="exam-scores-form" onSubmit={handleSubmit(onSubmit)} noValidate>
          {/* Hidden fields to keep all studentId registered */}
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-white">
                <tr className="border-b border-gray-100">
                  {["#", "Student Name", "Gender", `Score (/ ${exam.totalScore})`, "Rate"].map(
                    (col) => (
                      <th
                        key={col}
...
export default ExamScoresModal;
